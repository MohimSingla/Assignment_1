...
const getBooks = async (req, res) => {
  try {
    ...
    const pageCount = Math.floor(
      ((await Book.countDocuments({})) + (limit - 1)) / limit
    );
    if (pageCount < page + 1) {
      logger.error("Invalid data requested by the user.");
      throw new Error(
        "Total number of pages currently are: " +
          pageCount +
          ". Kindly access any page number lower than that."
      );
    }
...
const updateBookData = async (req, res) => {
  try {
    ...
    const _id = req.params.id;
    const updatedData = await Book.findOneAndUpdate({ _id }, req.body, {
      runValidators: true,
      new: true,
    });
    if (!updatedData) {
      ...
      throw new Error(
        "Book with the entered ID not found. Kindly try again with correct Information."
      );
    }
...
const deleteBookData = async (req, res) => {
  try {
    ...
    logger.info(
      "Sending 200 status code with relevant success message to the user."
    );
    ...
    res
      ...
      .send({
        error:
          "Unable to delete the requested book. Kindly verify the entered details or Please try again later if the error still persists!",
      });
  }
};
